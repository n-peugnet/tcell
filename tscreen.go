...
func LookupTerminfo(name string) (ti *terminfo.Terminfo, e error) {
	...
	if ti.Hyperlink == "" && ti.StringTerminator == "" {
		// Enable hyperlinks on all terminals, gomuks will only try to use them if enabled separately.
		ti.Hyperlink = "\x1b]8;"
...
func (t *tScreen) drawCell(x, y int) int {
	...
	if style != t.curstyle {
		...
		if style.hyperlink != "" && ti.Hyperlink != "" {
			if len(style.hyperlinkID) > 0 {
				t.TPuts(ti.Hyperlink + "id=" + style.hyperlinkID + ";" + style.hyperlink + ti.StringTerminator)
			} else {
